...
4. **`GET /api/order/{orderId}`** - 주문 상세 조회

### 멤버십 관련 핵심 API
5. **`GET /api/membership/user/{userId}/info`** - 멤버십 정보 및 등급 조회 (총 결제 금액 포함)
6. **`GET /api/membership/user/{userId}/payments`** - 사용자 결제 내역 조회

### 환불 관련 핵심 API
7. **`POST /api/refunds/request`** - 환불 요청 (포인트 환불 + 멤버십 등급 갱신)
8. **`POST /api/payments/cancel`** - 결제 취소 (PortOne)

### 포인트 관련 핵심 API
9. **`GET /api/points/balance/{userId}`** - 포인트 잔액 조회
10. **`POST /api/points/charge/{userId}`** - 포인트 충전
